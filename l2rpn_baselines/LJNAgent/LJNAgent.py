...
L2RPN_IDF_2023_DEFAULT_ACTION_SPACE = {
            ...
            "N1_interm": os.path.join(os.path.dirname(__file__), "action", "action_N1_interm.npz"),
            "N1_unsafe": os.path.join(os.path.dirname(__file__), "action", "action_N1_unsafe.npz"),
            "12_unsafe": os.path.join(os.path.dirname(__file__), "action", "action_12_unsafe.npz"),
        }
...
def load_action_to_grid2op(action_space, action_vec_path, action_threshold=None, return_counts=False):
    ...
    assert action_vec_path.endswith('.npz') and os.path.exists(action_vec_path), FileNotFoundError(f'file_paths {action_vec_path} does not contain a valid npz file.')
    data = np.load(action_vec_path, allow_pickle=True)
    all_actions = data['action_space']
    assert isinstance(all_actions, np.ndarray), RuntimeError(f'Expect {action_vec_path} to be an ndarray, got {type(all_actions)}')
    action_dim = action_space.size()
    assert all_actions.shape[1] == action_dim, RuntimeError(f'Expect {action_vec_path} to be an ndarray of shape {action_dim}, got {all_actions.shape[1]}')
    counts = data['counts']
...
class LJNAgent(BaseAgent):
    ...
    def __init__(self, 
                 ...
                 action_space_path_N1_interm : str =  L2RPN_IDF_2023_DEFAULT_ACTION_SPACE["N1_interm"], 
                 action_space_path_N1_unsafe : str =  L2RPN_IDF_2023_DEFAULT_ACTION_SPACE["N1_unsafe"], 
                 action_space_path_12_unsafe : str =  L2RPN_IDF_2023_DEFAULT_ACTION_SPACE["12_unsafe"], 
        ...
        self._load_topo_actions(action_space_path_N1_safe,action_space_path_N1_interm,action_space_path_N1_unsafe,action_space_path_12_unsafe)
        self.lines_in_area = [list_ids for list_ids in env._game_rules.legal_action.lines_id_by_area.values()]        
    ...
    def _init_params(self, env):
        ...
        self._margin_th_limit = cp.Parameter(value=self.config["margin_th_limit"], nonneg=True)
        self._penalty_curtailment_unsafe = cp.Parameter(value=self.config["penalty_curtailment_unsafe"], nonneg=True)
        self._penalty_redispatching_unsafe = cp.Parameter(value=self.config["penalty_redispatching_unsafe"], nonneg=True)
        self._penalty_storage_unsafe = cp.Parameter(value=self.config["penalty_storage_unsafe"], nonneg=True)
        self._penalty_curtailment_safe = cp.Parameter(value=self.config["penalty_curtailment_safe"], nonneg=True)
        self._penalty_redispatching_safe = cp.Parameter(value=self.config["penalty_redispatching_safe"], nonneg=True)
        self._penalty_storage_safe = cp.Parameter(value=self.config["penalty_storage_safe"], nonneg=True)
        self._weight_redisp_target = cp.Parameter(value=self.config["weight_redisp_target"], nonneg=True)
        self._weight_storage_target = cp.Parameter(value=self.config["weight_storage_target"], nonneg=True)
        self._weight_curtail_target = cp.Parameter(value=self.config["weight_curtail_target"], nonneg=True)
        self._alpha_por_error = cp.Parameter(value=self.config["alpha_por_error"], nonneg=True)
        ...
        self._powerlines_g = cp.Parameter(shape=powerlines_x.shape, value=1.0 * powerlines_g, pos=True)
        self._powerlines_b = cp.Parameter(shape=powerlines_x.shape, value=1.0 * powerlines_b, neg=True)   
        self._powerlines_ratio = cp.Parameter(shape=powerlines_x.shape, value=1.0 * powerlines_ratio, pos=True)
        ...
        self.load_per_bus = cp.Parameter(shape=self.nb_max_bus, value=1.0 * this_zeros_, nonneg=True)
        self.gen_per_bus = cp.Parameter(shape=self.nb_max_bus, value=1.0 * this_zeros_, nonneg=True)
        self.redisp_up = cp.Parameter(shape=self.nb_max_bus, value=1.0 * this_zeros_, nonneg=True)
        self.redisp_down = cp.Parameter(shape=self.nb_max_bus, value=1.0 * this_zeros_, nonneg=True)
        self.curtail_down = cp.Parameter(shape=self.nb_max_bus, value=1.0 * this_zeros_, nonneg=True)
        self.curtail_up = cp.Parameter(shape=self.nb_max_bus, value=1.0 * this_zeros_, nonneg=True)
        self.storage_down = cp.Parameter(shape=self.nb_max_bus, value=1.0 * this_zeros_, nonneg=True)
        self.storage_up = cp.Parameter(shape=self.nb_max_bus, value=1.0 * this_zeros_, nonneg=True)
    ...
    def _get_powerline_impedance(self, env):
        ...
        powerlines_x = np.array([float(el.x_pu) for el in line_info] + [float(el.x_pu) for el in trafo_info])
        powerlines_g = np.array([(1 / (el.r_pu + 1j * el.x_pu)).real for el in line_info] + [(1 / (el.r_pu + 1j * el.x_pu)).real for el in trafo_info])
        powerlines_b = np.array([(1 / (el.r_pu + 1j * el.x_pu)).imag for el in line_info] + [(1 / (el.r_pu + 1j * el.x_pu)).imag for el in trafo_info])
    ...
    def _load_topo_actions(self, action_space_path_N1_safe, action_space_path_N1_interm, action_space_path_N1_unsafe, action_space_path_12_unsafe):
        """
        ...
        - action_space_path_12_unsafe: File path for unsafe actions from greedy search for cas of attacking Lines and the overflow and reducing the load of the network.
        """
        self.topo_actions_to_check_N1_safe = load_action_to_grid2op(self.action_space, action_space_path_N1_safe)
        self.num_topo_actions_to_check_N1_safe = len(self.topo_actions_to_check_N1_safe)

        self.topo_actions_to_check_N1_interm = load_action_to_grid2op(self.action_space, action_space_path_N1_interm)
        self.num_topo_actions_to_check_N1_interm = len(self.topo_actions_to_check_N1_interm)
        
        self.topo_actions_to_check_N1_unsafe = load_action_to_grid2op(self.action_space, action_space_path_N1_unsafe)
        self.num_topo_actions_to_check_N1_unsafe = len(self.topo_actions_to_check_N1_unsafe)
        
        self.topo_actions_to_check_12_unsafe = load_action_to_grid2op(self.action_space, action_space_path_12_unsafe)
        self.num_topo_actions_to_check_12_unsafe = len(self.topo_actions_to_check_12_unsafe)
        ...
        try:
            with warnings.catch_warnings():
                ...
                if solver_type is cp.OSQP:
                    tmp_ = prob.solve( solver=solver_type, verbose=0, warm_start=False, max_iter=self.max_iter)
    ...
    def reset(self, obs: ObservationIDF2023):
        ...
        if conv_:
            ...
            self.logger.warning("Impossible to intialize the OptimCVXPY agent because the DC powerflow did not converge.")
    ...
    def compute_optimum_unsafe(self):
        ...
        for i in range(self.n_gen):
            ...
            if self.env.gen_renewable[i]:
                ...
                redispatching_constraints.append(redispatching[bus_id] <= self.env.gen_max_ramp_up[i])
                redispatching_constraints.append(redispatching[bus_id] >= -self.env.gen_max_ramp_down[i])
                
        # limit redispatching to possible values of redispatching
        redispatching_limit = [redispatching <= self.redisp_up, 
    ...
    def to_grid2op(self,obs: ObservationIDF2023,
                   curtailment: np.ndarray,
                   storage: np.ndarray,
                   redispatching: np.ndarray, 
                   base_action: BaseAction =None,safe=False) -> BaseAction:
        """
    ...
    def recon_line_area(self, obs):
        ...
        action_chosen_per_area = [self.action_space({}) for _ in self.lines_in_area]  # Default to "do nothing" actions
        ...
        if np.any(can_be_reco):
            actions_and_ids = [(self.action_space({"set_line_status": [(id_, +1)]}), id_) for id_ in np.where(can_be_reco)[0]]
            for action, id_ in actions_and_ids:
                obs_simu, _reward, _done, _info = obs.simulate(action, time_step=self.time_step)

                # Determine the area of the line using lines_in_area.
                area_id = next((idx for idx, lines in enumerate(self.lines_in_area) if id_ in lines), None)
                ...
                if area_id is not None and (obs_simu.rho.max() < min_rho_per_area[area_id]) & (len(_info['exception']) == 0):
    ...
    def act(self,observation: ObservationIDF2023,reward=None,done: bool=False):
        ...
        if observation.rho.max() > self.rho_danger:            
            ...
            logger.warning(f"{[time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())]}: step {observation.current_step} with rho: {round(float(observation.rho.max()),2)}, unsafe -- disconnection line: {np.where(observation.rho == 0)[0]}")
